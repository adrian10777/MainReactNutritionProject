...
const Navbar_ = props => {
  ...
  return (
    ...
    <Container>
      ...
      <Row>
        <Col id="navbartoggle">
        <Link className='nav-link' id="shopbutton2" to={props.cart.size ? '/cart' : '/shop'}><i className="fa fa-shopping-cart"></i> | {props.cart.size ? `${props.cart.size} | $${props.cart.total}` : 'Empty'}</Link>
          <Navbar expand={false}>
